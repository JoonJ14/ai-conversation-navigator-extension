...
!! For right now, I realized I will have to update this everytime we add a feature to original ai conversation navigation repo and then update here again. Since I don't just want an extension that shows ai agents, but only if we get to use our ai-conv-nav button that I created (as I think it is only right to use it with those features), I am going to pause this project until I think the original ai-conv-nav repo is at reasonable state where almost all features are added, and then I will work on this repo turning them into sidebars in the web browser. Also would need to turn it into official extension for all firefox, chrome, and safari, I think it is only right to go sequentially here, rather than making one feature and having to update that repo, this repo, and each of the official extension codes. I will return to this repo once the original one is almost all developed, or shouldn't add more features due to code getting heavy to run.
